...
<html>
	<head>
		...
		<spring:message code="general.hoyes"               var="hoy_es"/>
		...
		<script src="${pageContext.servletContext.contextPath}/js/comunes/sessionTimeout.js" type="text/javascript"></script>
		<title>${title}</title>
	...
	<div id="allContent">
		<div id="header">			
			<c:if test="${not empty LyFBean.logoCorporativo}">
				<c:if test="${not empty LyFBean.linkLogoCorporativo}">
			...
			<c:if test="${not empty LyFBean.logoCorporativo}">
			<c:if test="${empty LyFBean.linkLogoCorporativo}">
			...
			<br><br>
  			...
  			<c:if test="${not empty LyFBean.logoNombreApp}">
  				<c:if test="${not empty LyFBean.linkLogoNombreApp}">
				...
				<c:if test="${empty LyFBean.linkLogoNombreApp}">
				  					  					<img id="nameApplication" width="139" src="${pageContext.servletContext.contextPath}/lf/${LyFBean.lookAndFeel}/img/menu/${LyFBean.logoNombreApp}" alt="${ArchitechSession.nombreAplicacion}">
		...
		<div id="top03">
			<div class="left-container2">
				<c:if test="${not empty NewArchitechSession.nombre}">
					<b>${user}:&nbsp;</b>${NewArchitechSession.nombre}<br>
				</c:if> 
				<c:if test="${not empty NewArchitechSession.ultimoAcceso}">
					<b>${fechaUltimoAcceso}:&nbsp;</b>${NewArchitechSession.ultimoAcceso}<br>
				</c:if>
			</div>

			<div class="right-container2">
				${usuario}: ${NewArchitechSession.usuario} - ${hoy_es}: 
        		${Fecha_ES} <!-- - ${LyFBean.fecha} --> 
			</div>
...
</jsp:include><div id="content_container">		
